#!/bin/bash
# Script to check the fallback of CycleStreets
...
(
	flock -n 9 || { echo 'CycleStreets check fallback is already running' ; exit 1; }
...
# Do the checks
checkItemDone "Fallback update"

# Report
if [ -n "${inComplete}" ]; then
    message="The fallback update did not complete today.\n\nThe following items did not complete: ${inComplete}\n\nhttps://github.com/cyclestreets/cyclestreets-setup/blob/master/fallback-deployment/README.md#daily-update-did-not-complete\n\n\tYours,\n\n\t\tfallback cron"
    # Send mail
    recipientMail="${administratorEmail/webmaster/info}"
    echo -e ${message} | mail -s "Fallback cron did not complete today" ${recipientMail}
