...
if [ "${multipleEditions}" = 1 ]; then
    ...
    oldEditionDb=$(${superMysql} -s cyclestreets<<<"select getRoutingDbFromActiveAlias('${newEditionAlias}');")
...
if [ -n "${fallbackRoutingUrl}" -a "${multipleEditions}" = 1 ]; then
    ...
    ${superMysql} cyclestreets -e "call setRoutingEditionPriority('${newEdition}', ${oldEditionOrdering});";
    ${superMysql} cyclestreets -e "call activateRoutingEdition('${newEdition}');";
